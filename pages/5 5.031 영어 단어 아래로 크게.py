...
if 'question_generated' in st.session_state and st.session_state.question_generated:
    st.markdown("### 문제")
    st.write("빈칸에 들어갈 알파벳을 고르세요:")
    st.markdown(f'<p style="font-size: 24px; margin-top: 10px;">{st.session_state.blanked_word} {st.session_state.emoji}</p>', unsafe_allow_html=True)
