...
"""
    ...
    조건: 
    1. 문제의 정답은 1개입니다.  
    2. 영어 대화는 A와 B가 각각 1번씩 말하고 끝납니다.
    3. 질문 내용이 선택지에 직접적으로 포함되지 않도록 해주세요.
    4. 선택지는 대화 내용과 관련된 다양한 활동들로 구성해주세요.
    A는 다음과 같이 한문장을 말하세요.
    B는 다음과 같이 한문장을 말하세요.
    형식:
    [영어 대화]
...
def generate_explanation(question, correct_answer, user_answer, dialogue):
    ...
    이 학생에게 왜 그들의 답변이 틀렸는지, 그리고 정답이 무엇인지 짧고 간단하게 설명해주세요. 
    설명은 친절하고 격려하는 톤으로 작성해주세요. 
    대화의 내용을 참조하여 구체적으로 설명해주세요.
    단, 설명에서 질문이나 선택지의 내용을 그대로 반복하지 말고, 대화의 내용을 바탕으로 설명해주세요.
    """
...
st.subheader("직업을 묻고 답하는말 영어듣기 퀴즈👨‍🚀👩‍🚒")
st.divider()
