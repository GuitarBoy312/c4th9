...
def get_explanation(question, correct_answer, selected_option):
    ...
    예를 들면, "학생이 선택한 요리하다는 영어로 cooking 이예요. 하지만 문제에서는 I'm dancing, 즉 춤을 추고 있다고 했기 때문에 틀렸어요."
    ...
    if st.session_state.question_data:
        if st.session_state.question_type == "essay":
            ...
            for option in options:
                if st.checkbox(option, key=option, value=st.session_state.selected_option == option):
